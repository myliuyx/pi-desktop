...
## 五、主控复核结论（2026-09-23，M6 里程碑门禁）

**C6 全部行复核通过 ✅ —— M6（Pi 对接）里程碑完成。**
以下数字全部为**主控独立复跑**（非采信执行方回报），与 §三 基线一致：

| 项 | 主控复跑结果 |
|---|---|
| `check:c3` / c4 / c5 / **c6** | 全部通过 / 全部通过 / 全部通过 / **33/33**（`:false` 计数全 0） |
| `security-check` / `smoke:check` | 全部通过 / 全部通过 |
| core `tsc --noEmit -p` | EXIT=0 |
| `accept:m1` / m2 / m3 / m4 / m5 | 全通过（0 个 false）/ 32/32 / 15/15 / 16/16 / 21/21 |
| `check:cn` / `check:adapter` / `live:smoke` | 20/20 / 35 项 / 烟测全部通过 |
| ui `tsc --noEmit -p` | EXIT=0 |
| `vite build` + dist 隔离 | 10.51s；唯一命中仍是 mock 消息里的示例代码字符串 `from "@pi-coding-agent/sdk"`（C2 复核起已三次确认为非真实引用） |

**C6 三条遗留的处置确认**：
- §1.1 工具开关接 Pi：`check:c6` ⑥ 组断言通过（关 bash 后需 bash 的 prompt 不再出现 bash 调用）——**已实现并验证**；
- §1.2 `continue-recent` 重建：`check:c6` ⑤ 组断言通过（合成会话逼出真实切换、续写落同一文件）——**已实现并验证**（API 依据：0.87.1 公开选项 `sessionManager` + `sdk.js` 历史灌入路径，零私有 API）；
- §1.3 分支/fork UI：**确认记后期**（依据成立）。

**M6 收官**：`dev-m6` 分支交付「本地 core 服务 + 浏览器套壳」的可用真实链路
（对话 / 工具执行 / 授权门 / 信任门 / 历史会话 / 04·05 屏真数据 / 工具开关），
默认形态仍为 mock（`?empty=1`/`?stress=N`/`?mcp=1` 惯例照旧），全部既有验收期望值一行未改。
体验地址：`http://127.0.0.1:5190/?live=1`（core 起后免 token）。
