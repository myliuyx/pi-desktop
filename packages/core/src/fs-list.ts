 ...
 *
 * 定位：**只读列子目录**，给 UI 的「自定义路径」弹窗当浏览数据源。安全面零扩大：
 * 能拿到 token 的人本可经 agent 在这台机器上执行任意命令，列目录是严格更弱的能力
 * （deploy.md 安全边界段已补记）；端点进 API_ROUTES 自动获得 token + Host 白名单。
 *
 * 判定规则（契约见 task-dir-picker.md §4.0，UI 与 check 脚本两侧同源）：
 * - `~` 前缀展开 os.homedir()（path.resolve 不认 ~，而手输 ~ 是高频习惯；不支持 ~user）；
 * - 只列**目录**：dirent.isDirectory 直接收；符号链接 statSync 跟随判定，断链跳过
 *   （不可达的链接不是可选项，也不该让整个请求失败）；
...
export function listDirectories(raw: string, fallbackCwd: string): DirListOk {
	...
	for (const d of dirents) {
		...
		if (d.isSymbolicLink()) {
			try {
				if (fs.statSync(path.join(target, d.name)).isDirectory()) names.push(d.name);
			} catch {
				/* 断链 / 不可达：跳过，不让它拖垮整个列表 */
			}
		}
	...
	const entries = names.slice(0, DIR_LIST_MAX_ENTRIES).map((name) => ({ name, path: path.join(target, name) }));
